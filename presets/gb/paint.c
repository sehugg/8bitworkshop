/*
Paint program for Game Boy.
Ported from GBDK's examples/gb/paint.

A little drawing program: pick tools, colors and draw modes with
sprite menus and draw on the background with the gb/drawing.h
primitives (plot, line, box, circle).

The menu icons are stored as tile patterns and poked into VRAM with
switch_data(), which also saves the replaced tile so the menu can be
...
/*{w:8,h:8,bpp:1,count:48,brev:1,np:2,pofs:1,sl:2}*/
const unsigned char data[NB_DATA_TILES][16] =
{
  /* Pen */
...
/*{w:8,h:8,bpp:1,count:48,brev:1,np:2,pofs:1,sl:2}*/
const unsigned char selected_data[NB_DATA_TILES][16] =
{
  /* Pen */
...
const unsigned char cursors_data[][16] = {
  /* Arrow */
...
unsigned char data_buffer[NB_DATA_TILES][16];
