...
#define watchdog 0x8980

typedef struct pokey_t {
  struct {
    byte f; // freq
    byte c; // ctrl
...
#define pokey1 ((pokey_t*)0x6000)
#define pokey2 ((pokey_t*)0x6800)

const pokey_t* pokey[4] = {
  pokey1, pokey1, pokey2, pokey2
};
...
void play_music() {
  ...
  for (ch=0; ch<NV; ch++) {
    if (voice[ch].volume) {
      byte reg = (ch<<1)&2;
      pokey[ch]->aud[1+reg].c = 0xa0 | --voice[ch].volume;
    } else {
      freech = ch;
    }
  }
  if (music_ptr) {
	...
	byte reg = (ch<<1)&2;
        ...
        pokey[ch]->aud[0+reg].f = period;
        pokey[ch]->aud[1+reg].f = period >> 8;
        voice[ch].volume = 15;
        ch = ch ? ch-1 : NV-1;
      } else {
        if (note == 0xff)
...
void main() {
  music_ptr = 0;
  cur_duration = 0;
  pokey1->audctl = 0b01111000;
  pokey2->audctl = 0b01111000;
  while (1) {
    if (!music_ptr) start_music(music1);
    play_music();
    delay(300);
    POKE(watchdog, PEEK(watchdog)+1);
  }
}
