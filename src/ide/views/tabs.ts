...
function dbgTab(msg: string) {
  if (ASM_TAB_DEBUG) console.log("[asm-tab] " + msg);
}
...
function visualCol(text: string, index: number): number {
  let col = 0;
...
function addStop(col: number, freq: Map<number, number>) {
  if (col > 0) freq.set(col, (freq.get(col) || 0) + 1);
}
...
export function inferAsmTabStops(state: EditorState, pos: number): Map<number, number> {
  const cur = state.doc.lineAt(pos);
  const total = state.doc.lines;
  const scan = (from: number, to: number, step: number, freq: Map<number, number>): number => {
    let lines = 0;
...
function nextStop(freq: Map<number, number>, col: number, grid: number): number {
...
function prevStop(freq: Map<number, number>, col: number, grid: number): number {
  let best = -1, bestFreq = 0;
  for (const [s, f] of freq) {
    if (s < col && (f > bestFreq || (f == bestFreq && s > best))) {
      best = s; bestFreq = f;
...
function docHasTabs(state: EditorState, pos: number): boolean {
  const cur = state.doc.lineAt(pos);
...
function fmtStops(freq: Map<number, number>): string {
  return "[" + [...freq.entries()].sort((a, b) => a[0] - b[0]).map(([s, f]) => `${s}:${f}`).join(" ") + "]";
}

export function asmTabChange(state: EditorState, head: number): { from: number, insert: string } | null {
...
export function asmBackspaceRange(state: EditorState, head: number): { from: number, to: number } | null {
  const line = state.doc.lineAt(head);
...
function asmInsertSpaces(view: EditorView): boolean {
  const state = view.state;
  if (state.selection.ranges.some(r => !r.empty)) return false; // let indentMore handle selections
  const tr = state.changeByRange(range => {
    const change = asmTabChange(state, range.head);
    if (!change) return { range };
    return {
      changes: change,
      range: EditorSelection.cursor(range.head + change.insert.length),
    };
  });
  view.dispatch(tr);
  return true;
...
function asmDeleteSpacesBackward(view: EditorView): boolean {
  const state = view.state;
  ...
  view.dispatch(state.update({
    changes: del,
    selection: EditorSelection.cursor(del.from),
  }));
  return true;
}
...
export function asmSpacesKeymap(tabsToSpaces: () => boolean): KeyBinding[] {
  return [
