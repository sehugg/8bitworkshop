...
import { SymbolRecord, SymbolKind, SymbolSourceKind } from "../../common/searchtypes";
import { SearchHit } from "./types";

const SOURCE_WEIGHTS: Record<SymbolSourceKind, number> = {
  project: 1.3,
  toolchain: 1.0,
  docs: 0.7,
};
...
const KIND_WEIGHTS: Record<SymbolKind, number> = {
  func: 1.2,
  macro: 1.2,
  type: 1.2,
  struct: 1.2,
 ...
 */
function resolveClass(mod: any): any {
  ...
  // Named export
  if (typeof mod.MiniSearch === 'function') return mod.MiniSearch;
  if (typeof mod.uFuzzy === 'function') return mod.uFuzzy;
  return undefined;
}

export class SymbolIndex {
  private records: SymbolRecord[] = [];
  private nameHaystack: string[] = [];
  private msIds: Set<string> = new Set();
  private uf: any;
  private ms: any;
  private applySourceBoosts: boolean;
  private ufOptions: any;
  ...
  async init() {
    const ufMod = await import("@leeoniya/ufuzzy");
    const Ufuzzy = resolveClass(ufMod);
    if (Ufuzzy) {
      this.uf = new Ufuzzy(this.ufOptions);
    }

    const msMod = await import("minisearch");
  ...
  private addToMiniSearch(rec: SymbolRecord) {
    if (!this.ms || typeof this.ms.add !== 'function') return;
    if (this.msIds.has(rec.id)) return;
    try {
      this.ms.add(rec);
  ...
  pushRecords(records: SymbolRecord[]) {
  ...
  async setMiniSearch(msJSON: any) {
    const msMod = await import("minisearch");
    const MiniSearch = resolveClass(msMod);
  ...
  query(needle: string, limit: number): SearchHit[] {
    if (this.records.length === 0 || !needle) return [];
      ...
      try {
        ...
        if (srcIdxMap && order) {
          for (let i = 0; i < order.length; i++) {
            ...
            const score = base * SOURCE_WEIGHTS[record.source] * (KIND_WEIGHTS[record.kind] ?? 1);
            hits.push({ record, score, ranges: info.ranges?.[ufuzzyOrderIdx] });
          }
        }
      } catch (e) {
   ...
   */
  private baseScore(needle: string, info: any, i: number): number {
    const chars = info.chars?.[i] ?? 0;
    const interIns = info.interIns?.[i] ?? 0; // insertions between search terms
    const intraIns = info.intraIns?.[i] ?? 0; // insertions within terms (typos)
    const start = info.start?.[i] ?? 0;       // offset of first match char
    const interLft2 = info.interLft2?.[i] ?? 0;
    const interRgt2 = info.interRgt2?.[i] ?? 0;
    ...
    if (fullMatch && contiguous && prefixMatch) score = 1000;
    // Prefix match that's fully contiguous but missing chars (shorter needle)
    else if (contiguous && prefixMatch) score = 980;
    // Full but non-contiguous (spread across name)
    else if (fullMatch) score = 950;
    // Fuzzy
    else score = 900;
