...
export function expandTabs(s: string, tabsize: number = 8): string {
    let out = '';
    for (let i = 0; i < s.length; i++) {
        if (s[i] === '\t') {
            do { out += ' '; } while (out.length % tabsize);
        } else {
            out += s[i];
        }
    }
...
interface DASMMacro {
    line: number;   // line of the MAC directive
    file: string;   // file the macro was defined in
    len: number;    // lines from MAC to ENDM, 0 if we never saw the ENDM
}

interface DASMListingLine {
    linenum: number;
    offset: number;
    insns: string;
    rest: string;   // label + mnemonic + operands
    op: string;     // mnemonic + operands
}

// Split one listing line into its fields, or return null if it isn't one.
export function parseDASMListingLine(rawline: string): DASMListingLine | null {
...
export function parseDASMOutput(stdout: string, errors: WorkerError[], unresolved: {}): string {
...
export function dedupeErrors(errors: WorkerError[]): WorkerError[] {
    const seen = {};
