...
function parseXALabels(text: string) {
    const symbolmap = {};
...
function parseXAListing(lsttext: string, listings: CodeListingMap) {
    // xa prints the file name on its own line whenever it enters or resumes a file
    let path: string | null = null;
    let listing: CodeListing | null = null;
    let lastline = 0;
    const lastlines: { [path: string]: number } = {};
...
function getListingOrigin(listings: CodeListingMap): number | undefined {
    let origin: number | undefined;
    for (const key in listings) {
        for (const line of listings[key].lines) {
            if (origin === undefined || line.offset < origin) origin = line.offset;
        }
    }
    return origin;
}
...
export function assembleXA(step: BuildStep): BuildStepResult {
    const errors = [];
    ...
    if (staleFiles(step, [binpath])) {
        if (!wasiModule) {
            wasiModule = new WebAssembly.Module(loadWASMBinary("xa"));
        }
        ...
        const lstout = wasi.fs.getFile("./" + lstpath)?.getBytesAsString() || "";
        const symout = wasi.fs.getFile("./" + sympath)?.getBytesAsString() || "";
