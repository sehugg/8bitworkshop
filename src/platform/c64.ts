...
import { PLATFORMS, RAM, newAddressDecoder, padBytes, noise, setKeyboardFromMap, AnimationTimer, RasterVideo, Keys, makeKeycodeMap, dumpRAM, getMousePos, EmuHalt, KeyFlags } from "../emu";
...
class C64Platform extends Base6502Platform implements Platform {
  ...
  start() {
    ...
    // create video/audio
    this.video = new RasterVideo(this.mainElement, numVisiblePixels, numVisibleLines, {overscan:true});
  ...
  advance(novideo : boolean) {
    ...
    for (var sl=0; sl<linesPerFrame; sl++) {
      ...
      if (visible) {
        // do DMA for scanline?
        vicClocks -= this.vic.doDMA(this);
      }
      // copy line to frame buffer
      if (sl > firstScanline-24 && iofs < idata.length) {
  ...
  getDebugCategories() {
    return super.getDebugCategories().concat(['VIC-II']);
  }
