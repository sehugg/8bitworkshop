...
import { Bus, CPU, InstructionBased, SavesState, Interruptable, MemoryBusConnected } from "../devices";

export interface F8State {
  PC: number;   // alias for PC0 (required by CpuState)
  PC0: number;  // program counter
  PC1: number;  // saved program counter (stack)
  DC0: number;  // data counter
  DC1: number;  // alternate data counter
  A: number;    // accumulator
  W: number;    // status register (flags)
  IS: number;   // indirect scratchpad address register (6-bit)
  R: number[];  // 64-byte scratchpad RAM
...
export class F8CPU implements CPU, InstructionBased, SavesState<F8State>, Interruptable<number>, MemoryBusConnected {

  // Registers
  PC0: number = 0;     // active program counter
  PC1: number = 0;     // saved program counter
  DC0: number = 0;     // active data counter
  DC1: number = 0;     // alternate data counter
  A: number = 0;       // accumulator
  W: number = 0;       // status/flags
  IS: number = 0;      // indirect scratchpad address (6-bit)
  R: Uint8Array = new Uint8Array(64); // scratchpad RAM
  ...
  getPC(): number { return this.PC0; }
  getSP(): number { return this.PC1; }
  ...
  loadState(state: F8State): void {
    this.PC0 = state.PC0 ?? state.PC;
    this.PC1 = state.PC1;
    this.DC0 = state.DC0;
    this.DC1 = state.DC1;
    this.A = state.A;
    this.W = state.W;
    this.IS = state.IS;
    for (let i = 0; i < 64; i++) this.R[i] = state.R[i];
    this.o = state.o;
    this.irqRequest = state.irq;
  }

  // Read byte at PC0, increment PC0
  private fetchPC0(): number {
  ...
  private storeDC0(val: number): void {
    this.bus.write(this.DC0, val);
    this.DC0 = (this.DC0 + 1) & 0xFFFF;
  }

  // Clear O, Z, C, S flags
  private clrOZCS(): void {
  ...
  private setSZ(val: number): void {
    if ((val & 0xFF) === 0) this.W |= FLAG_Z;
    if ((val & 0x80) === 0) this.W |= FLAG_S;  // complementary sign
  }

  // Addition with carry/overflow detection
  private doAdd(a: number, b: number, carry: number = 0): number {
    const result = (a & 0xFF) + (b & 0xFF) + (carry & 1);
    // Set carry if overflow past 8 bits
    if (result & 0x100) this.W |= FLAG_C;
    // Set overflow using XOR method
    if (((a ^ result) & (b ^ result) & 0x80) !== 0) this.W |= FLAG_O;
    return result & 0xFF;
  }

  // BCD addition
  private doAddDecimal(augend: number, addend: number): number {
    ...
    let correction = 0;
    if (c2) correction |= 0xA0;
    if (c1) correction |= 0x0A;
    if (correction) {
      this.clrOZCS();
  ...
  private getScratchpadIndex(r: number): number {
    if (r < 12) return r;
    if (r === 12) return this.IS & 0x3F;
    if (r === 13) {
  ...
  advanceInsn(): number {
    ...
    switch (opcode) {
      // 0x00: LR A,KU
      case 0x00: this.A = this.R[12]; break;
      // 0x01: LR A,KL
      case 0x01: this.A = this.R[13]; break;
      // 0x02: LR A,QU
      case 0x02: this.A = this.R[14]; break;
      // 0x03: LR A,QL
      case 0x03: this.A = this.R[15]; break;
      // 0x04: LR KU,A
      case 0x04: this.R[12] = this.A; break;
      // 0x05: LR KL,A
      case 0x05: this.R[13] = this.A; break;
      // 0x06: LR QU,A
      case 0x06: this.R[14] = this.A; break;
      // 0x07: LR QL,A
      case 0x07: this.R[15] = this.A; break;
      ...
      // 0x0A: LR A,IS
      case 0x0A: this.A = this.IS & 0x3F; break;
      // 0x0B: LR IS,A
      case 0x0B: this.IS = this.A & 0x3F; break;
      ...
      case 0x8D:
        tmp = this.fetchDC0();
        this.clrOZCS();
        { const cmp = this.doAdd((~this.A) & 0xFF, tmp, 1); this.setSZ(cmp); }
        cycles = 6;
        break;
