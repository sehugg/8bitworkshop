...
function extractBannerSection(line: string): string | null {
...
function parseDocLines(lines: string[], sectionOverride?: string | null): DocInfo | null {
  ...
  if (!result.brief && !result.detail) return null;
  return result;
}

/** Extract doc info from a trailing comment on the same line */
export function extractTrailingComment(line: string): string | undefined {
