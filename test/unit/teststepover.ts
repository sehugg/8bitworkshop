...
class TestStepPlatform extends BaseDebugPlatform {
  mem = new Uint8Array(0x10000);
  cpu = { PC: 0, SP: 0xff };
  paused = false;

  getPC(): number { return this.cpu.PC; }
  getSP(): number { return this.cpu.SP; }
  getCPUState(): CpuState { return { PC: this.cpu.PC, SP: this.cpu.SP }; }
  isStable(): boolean { return true; }
  // uses 'this' -- must not be called unbound
  readAddress(a: number): number { return this.mem[a & 0xffff]; }
  // like real platforms, goes through the passed-in read callback
  disassemble(pc: number, read: (a: number) => number) {
    return disassemble6502(pc, read(pc), read(pc + 1), read(pc + 2));
  }
  saveState(): EmuState { return { c: { PC: this.cpu.PC, SP: this.cpu.SP } }; }
  loadState(state: EmuState) { this.cpu.PC = state.c.PC; this.cpu.SP = state.c.SP; }
...
describe('BaseDebugPlatform stepOver', function () {

  function makePlatform(): TestStepPlatform {
  ...
  it('should step over a JSR to the following instruction', function () {
    var p = makePlatform();
    var sp0 = p.getSP();
    p.stepOver();	// runs the runEval loop via advance()
    assert.strictEqual(p.getPC(), 0x0003);	// not inside the subroutine
    assert.strictEqual(p.getSP(), sp0);		// subroutine returned
  });

  it('should single-step when instruction is not a call', function () {
    var p = makePlatform();
    p.cpu.PC = 0x0003;	// NOP
    p.stepOver();
    assert.strictEqual(p.getPC(), 0x0004);
  });

  it('should detect iscall on JSR opcode', function () {
    var p = makePlatform();
    var d = p.disassemble(0x0000, (a) => p.readAddress(a));
    assert.strictEqual(d.iscall, true);
    assert.strictEqual(d.nbytes, 3);
  });
