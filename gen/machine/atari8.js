...
const atari8disk_1 = require("./atari8disk");
...
class Atari800 extends devices_1.BasicScanlineMachine {
    ...
    loadROM(rom, title) {
        if (rom[0] == 0x96 && rom[1] == 0x02) {
            ...
            this.disk = new atari8disk_1.DiskImage(rom);
            this.xexdata = null;
        ...
        if (disk == null) {
            ...
            switch (dcomnd) {
                case 0x53: { // STATUS
                    ...
                    for (let i = 0; i < n; i++)
                        this.write(dbuf + i, dvstat[i]);
                    break;
                }
                case 0x52: { // READ
                    if (dbyt == 0)
                        dbyt = disk.sectorsize;
                ...
                case 0x50: { // PUT
                    ...
                    for (let i = 0; i < dbyt; i++)
                        buf[i] = this.read(dbuf + i);
                    if (!disk.writeSector(sector, buf, dbyt))
                        status = 0x90;
                    break;
                }
                default:
