...
const ASTROCADE_KEYCODE_MAP = (0, emu_1.makeKeycodeMap)([
    // player 1
    [emu_1.Keys.UP, 0x10, 0x1],
    [emu_1.Keys.DOWN, 0x10, 0x2],
    [emu_1.Keys.LEFT, 0x10, 0x4],
    [emu_1.Keys.RIGHT, 0x10, 0x8],
    [emu_1.Keys.A, 0x10, 0x10],
    // player 2
    [emu_1.Keys.P2_UP, 0x11, 0x1],
    [emu_1.Keys.P2_DOWN, 0x11, 0x2],
    [emu_1.Keys.P2_LEFT, 0x11, 0x4],
    [emu_1.Keys.P2_RIGHT, 0x11, 0x8],
    [emu_1.Keys.P2_A, 0x11, 0x10],
    // keypad $14
    [emu_1.Keys.VK_P, 0x14, 0x1],
    [emu_1.Keys.VK_SLASH, 0x14, 0x2],
    [emu_1.Keys.VK_X, 0x14, 0x4],
    [emu_1.Keys.VK_MINUS, 0x14, 0x8],
    [emu_1.Keys.VK_COMMA, 0x14, 0x10],
    [emu_1.Keys.VK_EQUALS, 0x14, 0x20],
    // keypad $15
    [emu_1.Keys.VK_O, 0x15, 0x1],
    [emu_1.Keys.VK_L, 0x15, 0x2],
    [emu_1.Keys.VK_9, 0x15, 0x4],
    [emu_1.Keys.VK_6, 0x15, 0x8],
    [emu_1.Keys.VK_3, 0x15, 0x10],
    [emu_1.Keys.VK_PERIOD, 0x15, 0x20],
    // keypad $16
    [emu_1.Keys.VK_I, 0x16, 0x1],
    [emu_1.Keys.VK_K, 0x16, 0x2],
    [emu_1.Keys.VK_8, 0x16, 0x4],
    [emu_1.Keys.VK_5, 0x16, 0x8],
    [emu_1.Keys.VK_2, 0x16, 0x10],
    [emu_1.Keys.VK_0, 0x16, 0x20],
    // keypad $17
    [emu_1.Keys.VK_U, 0x17, 0x1],
    [emu_1.Keys.VK_J, 0x17, 0x2],
    [emu_1.Keys.VK_7, 0x17, 0x4],
    [emu_1.Keys.VK_4, 0x17, 0x8],
    [emu_1.Keys.VK_1, 0x17, 0x10],
    [emu_1.Keys.VK_BACK_SLASH, 0x17, 0x20],
]);
...
const _BallyAstrocade = function (arcade) {
    ...
    this.toLongString = (st) => {
        ...
        s += "\n  INLIN: " + st.inlin;
        s += "\n  VERBL: " + st.verbl;
        s += "\nMAGICOP: $" + (0, util_1.hex)(st.magicop);
        s += "\n  XPAND: $" + (0, util_1.hex)(st.xpand);
        s += "\nXPLOWER: " + st.xplower;
        s += "\n SHIFT2: $" + (0, util_1.hex)(st.shift2);
        s += "\n  HORCB: $" + (0, util_1.hex)(st.horcb);
        s += "\n  INMOD: $" + (0, util_1.hex)(st.inmod);
        s += "\n  INFBK: $" + (0, util_1.hex)(st.infbk);
        s += "\n  INTST: $" + (0, util_1.hex)(st.intst); // intercept status
        s += "\nPalette: ";
        for (var i = 0; i < 8; i++)
            s += (0, util_1.hex)(palinds[i]);
        s += "\n";
        return s;
    };
...
class BallyAstrocade extends devices_1.BasicScanlineMachine {
    constructor(arcade) {
        super();
    ...
    setPaddleInput(controller, value) {
        switch (controller) {
            case 0:
                this.inputs[0x1c] = value & 0xff;
                break;
            case 1:
                this.inputs[0x1d] = value & 0xff;
                break;
        }
    }
    startScanline() {
    ...
    loadBIOS(data) {
        this.m.loadBIOS((0, emu_1.padBytes)(data, 0x2000));
        this.reset();
    }
    reset() {
        super.reset();
...
class AstrocadeAudio extends audio_1.AY38910_Audio {
    setACRegister(addr, val) {
        addr &= 0x7;
        val &= 0xff;
        switch (addr) {
            case 0:
...
for (var i = 0; i < 256; i++) {
    ...
    ASTROCADE_PALETTE[i] = (0, util_1.rgb2bgr)(x) | 0xff000000;
}
/*
