...
const MCR2_KEYCODE_MAP = (0, emu_1.makeKeycodeMap)([
    [emu_1.Keys.SELECT, 0, 0x1], // Coin 1
    [emu_1.Keys.START, 0, 0x4], // 1P Start
    [emu_1.Keys.VK_2, 0, 0x8], // 2P Start
    [emu_1.Keys.UP, 1, 0x1], // P1 Up
    [emu_1.Keys.DOWN, 1, 0x2], // P1 Down
    [emu_1.Keys.LEFT, 1, 0x4], // P1 Left
    [emu_1.Keys.RIGHT, 1, 0x8], // P1 Right
    [emu_1.Keys.A, 1, 0x10], // P1 Button 1
    [emu_1.Keys.B, 1, 0x20], // P1 Button 2
    [emu_1.Keys.P2_UP, 2, 0x1], // P2 Up
    [emu_1.Keys.P2_DOWN, 2, 0x2], // P2 Down
    [emu_1.Keys.P2_LEFT, 2, 0x4], // P2 Left
    [emu_1.Keys.P2_RIGHT, 2, 0x8], // P2 Right
    [emu_1.Keys.P2_A, 2, 0x10], // P2 Button 1
    [emu_1.Keys.P2_B, 2, 0x20], // P2 Button 2
]);
// ROM blob layout
...
class MCR2Machine extends devices_1.BasicScanlineMachine {
    constructor() {
        super();
        this.cpuFrequency = MCR2_CPU_FREQ;
        this.canvasWidth = MCR2_CANVAS_WIDTH;
        this.numVisibleScanlines = MCR2_NUM_VISIBLE_SCANLINES;
        this.numTotalScanlines = MCR2_NUM_TOTAL_SCANLINES;
        this.cpuCyclesPerLine = MCR2_CYCLES_PER_LINE;
        this.defaultROMSize = ROM_TOTAL_SIZE;
        this.sampleRate = MCR2_FPS * MCR2_NUM_TOTAL_SCANLINES * 2;
        this.cpu = new ZilogZ80_1.Z80();
        this.ram = new Uint8Array(0x800); // E000-E7FF (NVRAM)
        this.sprram = new Uint8Array(0x200); // E800-E9FF (sprite RAM)
        this.vram = new Uint8Array(0x800); // F000-F7FF (video RAM)
        this.palram = new Uint8Array(0x80); // F800-F87F (palette RAM)
        ...
        this.audioadapter = new audio_1.TssChannelAdapter([this.psg1.psg, this.psg2.psg], 2, this.sampleRate);
        this.connectCPUMemoryBus(this);
        this.connectCPUIOBus(this.newIOBus());
        this.inputs.set([0, 0, 0, 0xff, 0xff]); // inputs + DIP switches
        this.handler = (0, emu_1.newKeyboardHandler)(this.inputs, MCR2_KEYCODE_MAP);
    }
    readConst(a) {
        return this.read(a);
    }
    ...
    drawSpriteScanline(scanline, pixofs) {
    ...
    loadROM(data) {
        this.rom = (0, emu_1.padBytes)(data, this.defaultROMSize);
        // Rebuild palette
        for (let i = 0; i < 64; i++) {
...
exports.MCR2Machine = MCR2Machine;
