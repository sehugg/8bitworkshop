...
exports.ChannelF = void 0;
const F8_1 = require("../common/cpu/F8");
...
class ChannelF extends devices_1.BasicMachine {
    constructor() {
        super();
        this.cpuFrequency = 1789773; // NTSC: 3.579545 MHz / 2
        this.defaultROMSize = 0x10000;
        // Video params
        this.canvasWidth = 128;
        this.numVisibleScanlines = 64;
        this.numTotalScanlines = 64;
        this.cpuCyclesPerLine = this.cpuFrequency / 60 / 64; // ~466 cycles per line
        this.sampleRate = this.cpuFrequency / 2; // audio sample rate
        this.cpu = new F8_1.F8CPU();
        this.ram = new Uint8Array(0x10000); // address space (ROM + VRAM)
        this.vram = new Uint8Array(0x2000); // 8KB video RAM (128x64, 2 bits per pixel)
        // I/O port latches
        this.port0 = 0; // console switches
        this.port1 = 0; // right controller
        this.port4 = 0; // left controller
        this.port5 = 0; // row/column/sound
        // Video registers
        this.row_reg = 0;
        this.col_reg = 0;
        this.color = 0;
        ...
        this.write = (0, emu_1.newAddressDecoder)([
            ...
            [0x2800, 0x2FFF, 0x07FF, (a, v) => { if (this.rom)
                    this.rom[a - 0x0800] = v; }],
        ], { gmask: 0xFFFF });
    ...
    setKeyInput(key, code, flags) {
        ...
        const pressed = !!(flags & emu_1.KeyFlags.KeyDown);
        const released = !!(flags & emu_1.KeyFlags.KeyUp);
        // Console buttons (mapped to number keys)
        if (key === emu_1.Keys.VK_1.c) { // TIME
            if (pressed)
                this.inputs[0] |= 0x01;
            else if (released)
                this.inputs[0] &= ~0x01;
        }
        if (key === emu_1.Keys.VK_2.c) { // MODE
            if (pressed)
                this.inputs[0] |= 0x02;
            else if (released)
                this.inputs[0] &= ~0x02;
        }
        if (key === emu_1.Keys.VK_3.c) { // HOLD
            if (pressed)
                this.inputs[0] |= 0x04;
            else if (released)
                this.inputs[0] &= ~0x04;
        }
        if (key === emu_1.Keys.VK_4.c) { // START
            if (pressed)
                this.inputs[0] |= 0x08;
            else if (released)
                this.inputs[0] &= ~0x08;
        ...
        if (key === emu_1.Keys.VK_UP.c || key === emu_1.Keys.VK_W.c) {
            if (pressed)
                this.inputs[1] |= INPUT_FORWARD;
            else if (released)
                this.inputs[1] &= ~INPUT_FORWARD;
        }
        if (key === emu_1.Keys.VK_DOWN.c || key === emu_1.Keys.VK_S.c) {
            if (pressed)
                this.inputs[1] |= INPUT_BACK;
            else if (released)
                this.inputs[1] &= ~INPUT_BACK;
        }
        if (key === emu_1.Keys.VK_LEFT.c || key === emu_1.Keys.VK_A.c) {
            if (pressed)
                this.inputs[1] |= INPUT_LEFT;
            else if (released)
                this.inputs[1] &= ~INPUT_LEFT;
        }
        if (key === emu_1.Keys.VK_RIGHT.c || key === emu_1.Keys.VK_D.c) {
            if (pressed)
                this.inputs[1] |= INPUT_RIGHT;
            else if (released)
                this.inputs[1] &= ~INPUT_RIGHT;
        }
        if (key === emu_1.Keys.VK_Q.c) { // CCW twist
            if (pressed)
                this.inputs[1] |= INPUT_CCW;
            else if (released)
                this.inputs[1] &= ~INPUT_CCW;
        }
        if (key === emu_1.Keys.VK_E.c) { // CW twist
            if (pressed)
                this.inputs[1] |= INPUT_CW;
            else if (released)
                this.inputs[1] &= ~INPUT_CW;
        }
        if (key === emu_1.Keys.VK_R.c) { // Push
            if (pressed)
                this.inputs[1] |= INPUT_PUSH;
            else if (released)
                this.inputs[1] &= ~INPUT_PUSH;
        }
        if (key === emu_1.Keys.VK_F.c) { // Pull
            if (pressed)
                this.inputs[1] |= INPUT_PULL;
            else if (released)
                this.inputs[1] &= ~INPUT_PULL;
        }
    }
