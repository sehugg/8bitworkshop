...
const PROBE_ARG_SEQS = [['--version'], [], ['-v']];
//// child mode state (only used inside --probe processes)
var childChunks = [];
var childModule = null;
var childReported = false;
function childCollect(line) {
    line = line.trim();
    if (line)
        childChunks.push(line);
}
function childFinish(status) {
...
function childReport(status) {
    if (childReported || !childModule)
        return;
    childReported = true;
...
async function probeWASI(moduleName, argv) {
...
async function runProbe(moduleName, argv) {
...
async function probeModule(moduleName) {
    ...
    for (var argv of PROBE_ARG_SEQS) {
        ...
        if (!best || (best.status === 'empty' && r.output))
            best = r;
        if (r.status === 'timeout' || r.status === 'missing')
            break;
    }
    return best;
}
//// parent mode: spawn one child per module, collect results
function probeInChild(moduleName, argv) {
    return new Promise((resolve) => {
        var child = (0, child_process_1.spawn)(process.execPath, [__filename, '--probe', moduleName, '--args', argv.join(',')], {
...
async function main() {
    ...
    if (probeIdx >= 0) {
        ...
        var probeArgs = (_b = (_a = args[args.indexOf('--args') + 1]) === null || _a === void 0 ? void 0 : _a.split(',')) !== null && _b !== void 0 ? _b : ['--version'];
    ...
    var byModule = {};
    for (var id of Object.keys(toolmeta_1.TOOL_META)) {
        var meta = toolmeta_1.TOOL_META[id];
        if (meta.wasmModule) {
    ...
    for (var r of results) {
        ...
        console.log((r.version ? '' : '! ') +
            ...
            'meta:' + (stored !== null && stored !== void 0 ? stored : '-') +
            (agree ? '' : '  MISMATCH') +
            '  (' + label + ')');
...
//# sourceMappingURL=toolversions.js.map