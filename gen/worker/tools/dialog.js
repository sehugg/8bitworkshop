...
exports.compileDialog = compileDialog;
...
async function compileDialog(step) {
    const errors = [];
    (0, builder_1.gatherFiles)(step, { mainFilePath: "main.dg" });
    const destpath = step.prefix + ".z8";
    if ((0, builder_1.staleFiles)(step, [destpath])) {
        if (!dialog_fs) {
            dialog_fs = await (0, wasiutils_1.loadWASIFilesystemZip)("dialog-fs.zip");
        }
        if (!wasiModule) {
            wasiModule = new WebAssembly.Module((0, wasmutils_1.loadWASMBinary)("dialogc"));
        }
