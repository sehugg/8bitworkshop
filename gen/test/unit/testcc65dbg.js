...
const cc65dbg_1 = require("../../src/worker/tools/cc65dbg");
const DBG = [
...
(0, mocha_1.describe)('cc65 debug file', function () {
    const { sizes, ignored } = (0, cc65dbg_1.parseCC65DbgSizes)(DBG, ['HEADER', 'CHARS']);
    (0, mocha_1.it)('sizes labels from the line spans that follow them', function () {
        assert_1.default.equal(sizes._formation, 32);
        assert_1.default.equal(sizes._attackers, 48);
        assert_1.default.equal(sizes._last, 1);
    });
    (0, mocha_1.it)('sums multiple spans up to the next label', function () {
        assert_1.default.equal(sizes._table, 8);
        assert_1.default.equal(sizes._next, 2);
    });
    (0, mocha_1.it)('uses sizes recorded by ca65', function () {
        assert_1.default.equal(sizes._main, 41);
    });
    (0, mocha_1.it)('omits duplicates, equates and labels without spans', function () {
        assert_1.default.ok(!('L0002' in sizes));
        assert_1.default.ok(!('FOO' in sizes));
        assert_1.default.ok(!('_nobytes' in sizes));
    });
    (0, mocha_1.it)('reports labels in ignored segments instead of sizing them', function () {
        assert_1.default.deepEqual(ignored, ['_chr_tiles']);
        assert_1.default.ok(!('_chr_tiles' in sizes));
