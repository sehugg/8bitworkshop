...
exports.PacmanMachine = void 0;
const ZilogZ80_1 = require("../common/cpu/ZilogZ80");
const devices_1 = require("../common/devices");
const emu_1 = require("../common/emu");
const PACMAN_KEYCODE_MAP = (0, emu_1.makeKeycodeMap)([
    [emu_1.Keys.UP, 0, 0x1],
    [emu_1.Keys.LEFT, 0, 0x2],
    [emu_1.Keys.RIGHT, 0, 0x4],
    [emu_1.Keys.DOWN, 0, 0x8],
    [emu_1.Keys.VK_5, 0, 0x20], // COIN1
    [emu_1.Keys.VK_6, 0, 0x40], // COIN2
    [emu_1.Keys.A, 0, 0x80], // FIRE (Space) — homebrew
    [emu_1.Keys.START, 1, 0x20], // START1
    [emu_1.Keys.VK_2, 1, 0x40], // START2
]);
const SCREEN_W = 224;
const SCREEN_H = 288;
/**
 ...
 */
class PacmanVideo {
    constructor(rom, vram, cram, ram) {
        this.rom = rom;
        this.vram = vram;
        this.cram = cram;
        this.ram = ram;
        this.tiles = new Uint8Array(256 * 64); // decoded 8x8 tiles, 2bpp index per pixel
        this.sprites = new Uint8Array(64 * 256); // decoded 16x16 sprites
        this.colors = new Uint32Array(32); // RGB from color PROM
        this.paletteRom = new Uint8Array(0x100); // 64 palettes × 4 pens
        this.spritePos = new Uint8Array(16); // coords at 0x5060-0x506f
        this.rebuild();
    }
    rebuild() {
    ...
    decodeStrip(input, inOff, output, bx, by, imgWidth) {
    ...
    drawTile(pixels, tileNo, pal, x, y) {
        if (x < 0 || x >= SCREEN_W)
            return;
    ...
    drawSprite(pixels, spriteNo, pal, x, y, flipX, flipY) {
        if (x <= -16 || x > SCREEN_W)
            return;
...
class PacmanMachine extends devices_1.BasicScanlineMachine {
    static buildWaveforms() {
    ...
    constructor() {
        ...
        this.canvasWidth = SCREEN_W;
        this.numTotalScanlines = SCREEN_H;
        this.numVisibleScanlines = SCREEN_H;
        this.defaultROMSize = 0x8000;
        this.cpuCyclesPerLine = cpuCyclesPerLine | 0;
        this.sampleRate = 60 * 264;
        this.rotate = 0;
        ...
        this.cram = new Uint8Array(0x400);
        this.oram = new Uint8Array(0x100); // kept for platform debug tree compat
        ...
        this.soundAcc = new Uint32Array(3);
        this.waveforms = PacmanMachine.WAVEFORMS;
        this.keyMap = PACMAN_KEYCODE_MAP;
        ...
        this.readByte = (a) => {
            ...
            if (a < 0x4000)
                return this.rom[a];
            if (a < 0x4400)
                return this.vram[a - 0x4000];
            if (a < 0x4800)
                return this.cram[a - 0x4400];
            // 0x4800-0x4BFF: open bus on real Pac-Man (MAME returns 0xBF / nop)
            if (a < 0x4c00)
                return 0xbf;
            if (a < 0x5000)
                return this.ram[a - 0x4c00];
            if (a < 0x5100) {
                var io = a & 0xc0;
                if (io === 0x00)
                    return ((~this.inputs[0]) & 0xff) | 0x10; // IN0
                if (io === 0x40)
                    return ((~this.inputs[1]) & 0x6f) | 0x90; // IN1 upright, service off
                if (io === 0x80)
                    return 0xc9; // DSW1
                return 0xff;
            }
            if (a >= 0x8000)
                return this.rom[a & 0x3fff];
            return 0xff;
        };
        this.writeByte = (a, v) => {
            a &= 0xffff;
            if (a < 0x4000)
                return;
            if (a < 0x4400) {
                this.vram[a - 0x4000] = v;
                return;
            }
            if (a < 0x4800) {
                this.cram[a - 0x4400] = v;
                return;
            }
            if (a < 0x4c00)
                return; // open bus — writes discarded (matches MAME nopw)
            if (a < 0x5000) {
                this.ram[a - 0x4c00] = v;
                return;
            }
    ...
    voiceVol(v) {
        if (v === 0)
            return this.soundRegs[0x15];
        if (v === 1)
            return this.soundRegs[0x1a];
        return this.soundRegs[0x1f];
    }
    voiceWave(v) {
        if (v === 0)
            return this.soundRegs[0x05] & 7;
        if (v === 1)
            return this.soundRegs[0x0a] & 7;
    ...
    read(a) { return this.readByte(a); }
    write(a, v) { this.writeByte(a, v); }
    readConst(a) { return this.readByte(a); }
}
exports.PacmanMachine = PacmanMachine;
// Built-in 8 × 32 4-bit waveforms (stand-in for missing 82s126.1m/.3m)
PacmanMachine.WAVEFORMS = PacmanMachine.buildWaveforms();
