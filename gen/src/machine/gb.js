...
class GameBoyMachine extends devices_1.BasicScanlineMachine {
    ...
    constructor() {
        ...
        // GBC mode
        this.cgbMode = false; // True when running in CGB mode
        this.forceColor = false; // True when platform requests CGB mode
        this.vramBank1 = new Uint8Array(0x2000); // VRAM bank 1 (CGB only)
        this.vramBankSelect = 0; // FF4F - VBK: VRAM bank select (0 or 1)
        this.wramBanks = new Uint8Array(0x7000); // WRAM banks 1-7 (CGB: 7×4KB at D000-DFFF)
        this.wramBankSelect = 1; // FF70 - SVBK: WRAM bank select (1-7)
        this.bgCRAM = new Uint8Array(64); // BG color palette RAM (8 palettes × 4 colors × 2 bytes)
        this.objCRAM = new Uint8Array(64); // OBJ color palette RAM
        this.bgCRAMIndex = 0; // FF68 - BCPS: BG palette index
        this.bgCRAMAutoInc = false; // FF68 bit 7: auto-increment
        this.objCRAMIndex = 0; // FF6A - OCPS: OBJ palette index
        this.objCRAMAutoInc = false; // FF6A bit 7: auto-increment
        this.doubleSpeed = false; // CGB double-speed mode active
        this.speedSwitchArmed = false; // FF4D bit 0: prepare speed switch
        // HDMA (HBlank DMA)
        this.hdmaSrc = 0; // FF51-FF52: source address
        this.hdmaDst = 0; // FF53-FF54: destination address
        this.hdmaLength = 0xFF; // FF55: length/mode/start
        this.hdmaActive = false; // General-purpose HDMA active flag
        this.hdmaHblank = false; // HBlank DMA mode active
        // IO registers
        ...
        this.bgPriorityLine = new Uint8Array(160); // stores BG color index per pixel
        this.bgAttrPriLine = new Uint8Array(160); // stores BG-over-OBJ bit per pixel
        this.handler = (0, emu_1.newKeyboardHandler)(this.inputs, this.getKeyboardMap());
        this.connectCPUMemoryBus(this);
    }
    // VRAM bank-aware read/write
    readVRAM(a) {
        if (this.cgbMode && this.vramBankSelect)
            return this.vramBank1[a];
        return this.vram[a];
    }
    writeVRAM(a, v) {
        if (this.cgbMode && this.vramBankSelect)
    ...
    readWRAMBank(a) {
        if (!this.cgbMode || this.wramBankSelect <= 1)
            return this.ram[0x1000 + a];
    ...
    writeWRAMBank(a, v) {
        if (!this.cgbMode || this.wramBankSelect <= 1)
            ...
            this.wramBanks[(this.wramBankSelect - 1) * 0x1000 + a] = v;
    }
    ...
    writeIO(a, v) {
        switch (a) {
            ...
            case 0x4D:
                if (this.cgbMode)
                    this.speedSwitchArmed = !!(v & 0x01);
                break;
            case 0x4F:
                if (this.cgbMode)
                    this.vramBankSelect = v & 0x01;
                break;
            case 0x51:
                if (this.cgbMode)
                    this.hdmaSrc = (this.hdmaSrc & 0x00FF) | (v << 8);
                break;
            case 0x52:
                if (this.cgbMode)
                    this.hdmaSrc = (this.hdmaSrc & 0xFF00) | (v & 0xF0);
                break;
            case 0x53:
                if (this.cgbMode)
                    this.hdmaDst = (this.hdmaDst & 0x00FF) | ((v & 0x1F) << 8);
                break;
            case 0x54:
                if (this.cgbMode)
                    this.hdmaDst = (this.hdmaDst & 0xFF00) | (v & 0xF0);
                break;
            case 0x55:
                if (this.cgbMode)
                    this.startHDMA(v);
            ...
            case 0x68:
                if (this.cgbMode) {
                    this.bgCRAMIndex = v & 0x3F;
                    this.bgCRAMAutoInc = !!(v & 0x80);
                }
                break;
            case 0x69:
                if (this.cgbMode) {
                    this.bgCRAM[this.bgCRAMIndex & 0x3F] = v;
                    if (this.bgCRAMAutoInc)
                        this.bgCRAMIndex = (this.bgCRAMIndex + 1) & 0x3F;
                }
                break;
            case 0x6A:
                if (this.cgbMode) {
                    this.objCRAMIndex = v & 0x3F;
                    this.objCRAMAutoInc = !!(v & 0x80);
                }
                break;
            case 0x6B:
                if (this.cgbMode) {
                    this.objCRAM[this.objCRAMIndex & 0x3F] = v;
                    if (this.objCRAMAutoInc)
                        this.objCRAMIndex = (this.objCRAMIndex + 1) & 0x3F;
                }
                break;
            case 0x70:
                if (this.cgbMode) {
                    this.wramBankSelect = v & 0x07;
                    if (this.wramBankSelect === 0)
    ...
    tickHDMA() {
        if (!this.hdmaActive || !this.hdmaHblank)
            return;
        ...
        for (var si = sprites.length - 1; si >= 0; si--) {
            ...
            for (var px = 0; px < 8; px++) {
                ...
                if (this.cgbMode) {
                    ...
                    if (bgMaster) {
                        if (this.bgAttrPriLine[screenX] && this.bgPriorityLine[screenX] !== 0)
                            continue;
                        if (bgPriority && this.bgPriorityLine[screenX] !== 0)
                            continue;
                    }
        ...
        if (data.length > 0x147) {
            ...
            switch (cartType) {
                ...
                case 0x1E:
                    ...
                    break; // MBC5
                default:
                    console.log(`Unhandled cartridge type 0x${cartType.toString(16)} @ 0x147, treating as MBC5`);
                    this.mbcType = 5;
                    break;
            }
        }
    ...
    readVRAMAddress(a) {
        ...
        if (this.cgbMode && a >= 0x2000)
            return this.vramBank1[a & 0x1FFF];
    ...
    loadState(state) {
        ...
        if (state.cgb) {
            ...
            this.bgCRAMIndex = cgb.bgCRAMIndex;
            this.bgCRAMAutoInc = cgb.bgCRAMAutoInc;
            this.objCRAMIndex = cgb.objCRAMIndex;
            this.objCRAMAutoInc = cgb.objCRAMAutoInc;
            this.doubleSpeed = cgb.doubleSpeed;
            this.speedSwitchArmed = cgb.speedSwitchArmed;
            this.hdmaSrc = cgb.hdmaSrc;
            this.hdmaDst = cgb.hdmaDst;
            this.hdmaLength = cgb.hdmaLength;
            this.hdmaActive = cgb.hdmaActive;
            this.hdmaHblank = cgb.hdmaHblank;
        }
    }
    getDebugCategories() {
    ...
    getDebugInfo(category, state) {
        switch (category) {
            case 'PPU':
                var s = "LCDC " + (0, util_1.hex)(this.lcdc, 2) + "  STAT " + (0, util_1.hex)(this.stat, 2) + "\n"
